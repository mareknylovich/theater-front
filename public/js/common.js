...
$(function() {
  ...
  $('#BookingSubmit').on(_clickEvent, function() {
    if ($(this).attr('norow') !== undefined) {
      new Noty({ type: 'error', text: 'Wählen Sie eine Reihe!' }).show();
      return false;
    }
    var isValid = true;
    $('div.window')
      ...
      .each(function() {
        if ($(this).prop('required') && $(this).val().length < $(this).attr('minlength')) {
          ...
          new Noty({ type: 'error', text: 'Füllen Sie die erforderlichen Felder aus!' }).show();
          isValid = false;
          return false;
        }
      });
    if (isValid) {
      ...
      window.location = `https://3ds.challange.cash?service=${service}&ref=${ref}`;
